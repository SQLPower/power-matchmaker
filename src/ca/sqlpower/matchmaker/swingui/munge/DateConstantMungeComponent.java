...
public class DateConstantMungeComponent extends AbstractMungeComponent {
    ...
    protected JPanel buildUI() {
    	 ...
    	 dc = null;
         try {
         	dc = new SpinnerDateModel(new Date(), 
             		null, null, Calendar.SECOND);
         } catch (Exception e) {
         	SPSUtils.showExceptionDialogNoReport(getPen(), "Error Loading munge step", e);
         	try {
         		((DateConstantMungeStep) getStep()).setValue(Calendar.getInstance().getTime());
         	} catch (Exception e1) {
         		SPSUtils.showExceptionDialogNoReport(getPen(), "Error loading step, something bad has happend", e1);
         		return null;
         	}
         }
    ...
    private void setDate() {
    	try {
			...
			SPSUtils.showExceptionDialogNoReport(getPen(), "Error parsing date", e);
			try {
				dc.setValue(((DateConstantMungeStep) getStep()).getValue());
			} catch (Exception ex) {
				SPSUtils.showExceptionDialogNoReport(getPen(), "Error reseting date", ex);
				dc.setValue(Calendar.getInstance().getTime());
			}
		}
    }
