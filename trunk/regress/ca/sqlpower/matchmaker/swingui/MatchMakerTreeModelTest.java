...
public class MatchMakerTreeModelTest extends TestCase {
	...
	private final PlFolder<TestingAbstractMatchMakerObject> folder = new PlFolder<TestingAbstractMatchMakerObject>("Test Folder");
	private final TestingAbstractMatchMakerObject mmo = new TestingAbstractMatchMakerObject();
	...
	public void testTreeNodeInsertEvent() {
		...
		assertEquals("Last event source should be current folder",
				currentFoldersNode,counter.getLastEvt().getSource());
	}
	...
	public void testTreeNodeRemoveEvent() {
		...
		folder.addChild(mmo);
		treeModel.addTreeModelListener(counter);
		mmo.getEventSupport().fireChildrenRemoved("property name",new int[]{0}, Arrays.asList(new MatchMakerObject[] {mmo}));
	...
	public void testTreeNodeChangeEvent() {
		...
		folder.addChild(mmo);
		treeModel.addTreeModelListener(counter);
		mmo.getEventSupport().firePropertyChange("property name","old value","new value");
