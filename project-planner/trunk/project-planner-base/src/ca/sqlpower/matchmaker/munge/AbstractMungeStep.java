...
/**
 * An abstract implementation of the MungeStep interface. The only method that
 * extending classes are required to implement would be the {@link #call()}
 * method, which would implement the functionality of a particular MungeStep.
 */
public abstract class AbstractMungeStep extends
        AbstractMatchMakerObject<MungeStep, MungeStepOutput> implements
        MungeStep {
    ...
    /**
     * A list of Input objects, each containing a MungeStepOutput that the
     * parent MungeStep has output, which this MungeStep will use for its input.
     */
    ...
    /**
     * A URL of a web page that gives more information about a step to the user.
     */
    private String infoURL;

    /**
     * The default object type of this Munge Step's input. The default value is
     * {@link Object#class}. This is used for Munge Steps with variable inputs
     * as the default class to use when adding an new input which now happens
     * when connecting an input into the last empty input. When extending the
     * AbstractMungeStep, if your expected input object type isn't Object (for
     * example, {@link ConcatMungeStep} expects String), then use the
     * {@link #setDefaultInputClass(Class)} to set your expected input type.
     */
    ...
    /**
     * A method that is called when a step is attempting to rollback. Default is
     * No-op.
     */
    ...
    /**
     * A method that is called when a step is trying to commit. Default is
     * No-op.
     */
    ...
    public void connectInput(int index, MungeStepOutput<?> o) {
        if (index >= getMSOInputs().size()) {
            throw new IndexOutOfBoundsException("There is no input at index "
                    + index + " (inputs.size = " + getMSOInputs().size() + ")");
        }
        if (!inputs.get(index).getType().isAssignableFrom(o.getType())) {
            throw new UnexpectedDataTypeException("Input " + index
    ...
    /**
     * Disconnects the input at the given index by removing the MungeStepOutput,
     * note that this may remove more than one input
     */
    ...
    public void removeInput(int index) {
        ...
        inputs.remove(index);
        getEventSupport().firePropertyChange("addInputs", index, old, null);
    }
    ...
    public void removeUnusedInput() {
        try {
            ...
            while (inputs.get(inputs.size() - 1).getCurrent() == null
                    && inputs.size() > 1) {
    ...
    /**
     * Warning: One should never mix instances--that is, one should never put
     * detached instances from different sessions into the same set because it
     * will break this implementation of equals and hashCode.
     */
    @Override
    ...
    /**
     * This is the pairing between a MungeStepOutput value and its
     * InputDescriptor. The reason for this class is to avoid maintaining two
     * separate collections of MungeStepOutputs and their InputDescriptors.
     */
    public static class Input {
        ...
        public Input(MungeStepOutput current, InputDescriptor descriptor,
                AbstractMungeStep step) {
        ...
        /**
         * Returns the parent step this input belongs to.
         */
        public AbstractMungeStep getParentStep() {
            return parentStep;
        }

        /**
         * Sets the parent step this input belongs to. This private method is
         * called by Hibernate, so don't remove it even if it appears unused.
         */
        @SuppressWarnings("unused")
        ...
        /**
         * Returns the MungeStepOutput that is currently connected to this
         * input.
         */
        ...
        /**
         * Sets the MungeStepOutput that is currently connected to this input.
         * Only used by hibernate and persistence layers. Regular code should
         * only use addInput/connectInput.
         */
        ...
        /**
         * Sets the data type accepted by this input.
         */
    ...
    /**
     * Replaces the current child list with the given list, firing a Structure
     * Changed event. This method is exposed as public for the benefit of the
     * persistence layer. Don't use it unless you're working on MatchMakerObject
     * persistence!
     */
    public void setChildren(List<MungeStepOutput> children) {
    ...
    /**
     * Only sets the logger, because most steps do not need to allocate any
     * resources. If your step needs to allocate resources (perform a database
     * query, open a file, connect to a server, and so on), you should override
     * this method.
     */
    public final void open(Logger logger) throws Exception {
        this.logger = logger;
        if (logger == null) {
            throw new NullPointerException("Step " + getClass().getName()
                    + " was given a null logger");
        }
    ...
    /**
     * Called when the step tries to commit. This called doCommit(). doCommit
     * should be overridden if the step is to do anything when commit is called.
     */
    public final void commit() throws Exception {
        if (!opened) {
            throw new IllegalStateException(
                    "Can't commit because step is not opened");
        }
        if (committed || rolledBack) {
            throw new IllegalStateException(
                    "Can't commit because step is already committed or rolled back"
                            + " (committed=" + committed + "; rolledBack="
                            + rolledBack + ")");
        }
        doCommit();
        committed = true;
    ...
    /**
     * Called when the step tries to rollback. This called doRollback().
     * doRollback() should be overridden if the step is to do anything when
     * commit is called.
     */
    public final void rollback() throws Exception {
        if (!opened) {
            throw new IllegalStateException(
                    "Can't roll back because step is not opened");
        }
        if (committed || rolledBack) {
            throw new IllegalStateException(
    ...
    /**
     * Called when this step is closed. This calls doClose(). doClose should be
     * overridden if the step needs to do anything on close.
     */
    public final void close() throws Exception {
        ...
        if (logger == null) {
            System.err.println("Warning: Step " + getClass().getName()
                    + " lost its logger");
        } else {
            if (logger.isDebugEnabled()) {
    ...
    /**
     * Prints all the inputs iff the given logger has debugging enabled
     */
    ...
    /**
     * Prints all the outputs iff the given logger has debugging enabled
     */
     ...
     * over written. This step calls doCall(), any subclass should override
     * doCall if the step is to do anything.
     */
    public final Boolean call() throws Exception {
        if (!opened) {
            throw new IllegalStateException(
                    "A munge step must be opened before it is called.");
        }
    ...
    /**
     * Returns the first MungeStepOutput it finds with the given name. Returns
     * null if no such MungeStepOutput exists.
     */
    ...
    /**
     * Returns the MMO ancestor of this munge step that is a Project. Returns
     * null if there is no such ancestor.
     */
    public Project getProject() {
        for (MatchMakerObject<?, ?> mmo = getParent(); mmo != null; mmo = mmo
                .getParent()) {
    ...
    /**
     * only used by hibernate and persistence layer.
     */
    ...
    /**
     * Sets the expected input object type for new Inputs. Typically used by
     * classes that extend AbstractMungeStep and expect a default Input class
     * that is not Object. It is currently default access for now, since all
     * Munge Steps are placed in the ca.sqlpower.matchmaker.munge package
     * anyway.
     */
